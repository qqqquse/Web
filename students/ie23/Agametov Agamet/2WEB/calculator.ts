...
class Calculator {
  ...
  private readonly MAX_INPUT_LENGTH: number = 16;
  private readonly MAX_HISTORY: number = 5;
  private readonly displayColors: string[] = ['#222', '#fff', '#f00', '#0f0', '#00f', '#ff0', '#f0f', '#0ff'];
  private currentDisplayColorIndex: number = 0;

  constructor(displayId: string) {
    this.display = document.getElementById(displayId) as HTMLDivElement;
  ...
  private initializeButtons(): void {
    ...
    buttons.forEach(button => {
      button.addEventListener('click', () => this.handleButtonClick(button.textContent?.trim() || ''));
    });
  }
  ...
  private mapKeyToOperation(key: string): Operation {
    switch (key) {
  ...
  private calculate(): void {
    if (this.operation === Operation.None || this.previousInput === '') return;
  ...
  private add(a: number, b: number): number { return a + b; }
  private subtract(a: number, b: number): number { return a - b; }
  private multiply(a: number, b: number): number { return a * b; }
  ...
  private percent(a: number, b: number): number { return a * (b / 100); }

  private applyUnaryOperation(op: Operation): void {
    ...
    this.currentInput = this.formatResult(result as number);
  ...
  private cycleDisplayColor(): void {
