import { SortType } from '../modules/types.js';

export class SortFilterComponent {
  private root: HTMLElement;
  private onChange: (sort: SortType | undefined) => void;

  constructor(root: HTMLElement, onChange: (sort: SortType | undefined) => void) {
    this.root = root;
    this.onChange = onChange;
  }

  render(currentSort?: SortType): void {
    this.root.innerHTML = `
      <div class="mb-3">
    ...
    const select = this.root.querySelector('#sortSelect') as HTMLSelectElement;
    select.addEventListener('change', () => {
      const value = select.value as SortType | '';
